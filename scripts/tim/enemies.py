...
class Enemy(Character):
    def __init__(self):
        ...
        self.lastDirection="left"#enemy starts at the right side of the screen
        self.left=True
        self.speed=3
        self.velocity=[-self.speed,0]
        self.standing=pygame.image.load(join(game_assets,"L1E.png"))
        self.surface=self.standing
        self.rect=self.surface.get_rect()
        self.rect.right=SCREEN_DIMENSIONS[0]
        self.rect.bottom=SCREEN_DIMENSIONS[1]
        self.walkRight=[pygame.image.load(join(game_assets,f"R{i}E.png")) for  i in range(1,12)]
        self.walkLeft=[pygame.image.load(join(game_assets,f"L{i}E.png")) for i in range(1,12)]
    def move(self):
        if self.rect.left==0 or self.rect.right==SCREEN_DIMENSIONS[0]:
...
pygame.time.set_timer(ADD_ENEMY,random.randint(minute/5,minute/4))
running=True
...
while running:
    ...
    for enemy in ENEMIES:
        ...
        if pygame.sprite.spritecollideany(enemy,player1.projectiles):
            enemy.kill()
            for projectile in player1.projectiles:
